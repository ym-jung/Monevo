...
import { createInviteRepository } from "./repository/invite-repository.ts";
import { createLedgerRepository } from "./repository/ledger-repository.ts";
import { createMemberRepository } from "./repository/member-repository.ts";
...
import { createInviteService } from "./service/invite-service.ts";
import { createLedgerService } from "./service/ledger-service.ts";
import { createMemberService } from "./service/member-service.ts";
...
function repositoriesOn(executor: Executor) {
	return {
		ledgers: createLedgerRepository(executor),
		members: createMemberRepository(executor),
		invites: createInviteRepository(executor),
...
export function buildServices(db: Database) {
	...
	const access = createAccessChecker(repositories.ledgers, repositories.members, repositories.scopes);
	...
	return {
		...
		members: createMemberService(repositories.members, repositories.scopes, access),
