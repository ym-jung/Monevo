...
import { createEntryRepository } from "./repository/entry-repository.ts";
import { createFxRepository } from "./repository/fx-repository.ts";
import { createScopeRepository } from "./repository/scope-repository.ts";
import { createRoutes } from "./routes.ts";
import { createJournalService } from "./service/journal-service.ts";
...
function repositoriesOn(executor: Executor) {
	return {
		entries: createEntryRepository(executor),
		scopes: createScopeRepository(executor),
		fx: createFxRepository(executor),
	};
}
...
export function buildServices(db: Database) {
	...
	return {
		journal: createJournalService(
			...
			repositories.scopes,
			repositories.fx,
			(run) => transaction(db, (tx) => run(repositoriesOn(tx))),
		),
	};
}
...
export function createApp(deps: AppDeps): Hono<{ Variables: AuthVariables }> {
	...
	app.use("/api/v1/*", byUser(deps.limiter, "fx"));
